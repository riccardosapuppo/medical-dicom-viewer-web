...
type StudyPrefetcherConfig = {
   ...
   * SmartImageLoadManager handles abort of in-flight requests when a new
   * series is loaded on a viewport, so this value can be tuned without
   * worrying about blocking new series loads.
   * */
...
class StudyPrefetcherService extends PubSubService {
  ...
  private config: StudyPrefetcherConfig = {
    ...
    /**
     * Max number of concurrent prefetch requests.
     * SmartImageLoadManager aborts these when a series change occurs.
     * */
  ...
  private async _sendNextRequests() {
    ...
    // Resolve the active viewport ID so SmartImageLoadManager can track
    // which viewport these prefetch requests belong to (enables abort-on-series-change).
    const { viewportGridService } = this._servicesManager.services;
    ...
    imageRequests.forEach(imageRequest => {
      ...
      const options = {
        ...
        additionalDetails: { imageId, viewportId: activeViewportId },
        preScale: {
          enabled: true,
        },
      };
      ...
      this.imageLoadPoolManager.addRequest(
        ...
        this.requestType,
        { imageId, viewportId: activeViewportId }
      );
