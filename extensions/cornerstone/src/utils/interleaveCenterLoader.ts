...
export default function interleaveCenterLoader({
  ...
  // Collect all viewportIds that will use these volumes so SmartImageLoadManager
  // can abort them on series change.
  const allViewportIds = Array.from(viewportIdVolumeInputArrayMap.keys());
  ...
  finalRequests.forEach(({ callLoadImage, additionalDetails, imageId, imageIdIndex, options }) => {
    ...
    // Inject viewportId into additionalDetails for SmartImageLoadManager tracking
    const enrichedDetails = {
      ...additionalDetails,
      viewportId: allViewportIds[0] || '',
    };
    imageLoadPoolManager.addRequest(callLoadImageBound, requestType, enrichedDetails, priority);
  });
