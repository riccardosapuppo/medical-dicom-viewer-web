...
function ViewportImageSliceLoadingIndicator({ viewportData, element }) {
  ...
  if (loading) {
    return (
      ...
      // interacting with the mouse, since scrolling should propagate to the viewport underneath
      <div>
        <div className="mdv-scrolling-loading pointer-events-none absolute top-0 left-0 h-full bg-black">
          <div className="transparent flex h-full w-full items-start justify-end"></div>
        </div>
        <div>
          <p className="mdv-scrolling-loading-p text-primary-light text-xl font-light">🟡</p>
        </div>
      </div>
    );
  }
