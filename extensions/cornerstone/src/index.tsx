...
const MontageComponent = React.lazy(() => {
  return import(/* webpackPrefetch: true */ './Viewport/Montage/OHIFMontageViewport');
});

const OHIFMontageViewport = props => {
  return (
    <React.Suspense fallback={<div>Loading...</div>}>
      <MontageComponent {...props} />
    </React.Suspense>
  );
};
...
const cornerstoneExtension: Types.Extensions.Extension = {
  ...
  onModeExit: ({ servicesManager }: withAppTypes): void => {
    ...
    // Abort all non-interaction in-flight requests and clear queues
    smartImageLoadManager.abortAllNonInteraction();
  ...
  getViewportModule({ servicesManager, commandsManager }) {
    ...
    return [
      ...
      {
        name: 'montage',
        component: ExtendedOHIFMontageViewport,
      },
    ];
  },
  getCommandsModule,
